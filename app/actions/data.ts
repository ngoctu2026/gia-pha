...
export async function exportData(
  ...
  const { data: allPersons, error: personsError } = await supabase
    .from("persons")
    .select(
      "id, full_name, gender, birth_year, birth_month, birth_day, death_year, death_month, death_day, death_lunar_year, death_lunar_month, death_lunar_day, is_deceased, is_in_law, birth_order, generation, other_names, avatar_url, note, hometown, grave_address, created_at, updated_at",
    )
  ...
  const { data: allClanNews, error: clanNewsError } = await supabase
    .from("clan_news")
    .select("id, title, summary, content, is_pinned, published_at, created_by")
    .order("is_pinned", { ascending: false })
    .order("published_at", { ascending: false });

  if (clanNewsError)
    return {
      error: "Lỗi tải dữ liệu clan_news: " + clanNewsError.message,
    };
  ...
  const exportClanNews = (allClanNews ?? []) as ClanNewsExport[];
  ...
  return {
    version: 4, // v4: adds hometown/grave_address and clan_news
...
export async function importData(
  ...
  const { error: delNewsError } = await supabase
    .from("clan_news")
    .delete()
    .neq("id", "00000000-0000-0000-0000-000000000000");
  ...
  if (delNewsError)
    return {
      error: "Lỗi khi xoá clan_news cũ: " + delNewsError.message,
    };
  ...
  // 9. Insert clan_news (if present in payload, strip created_by)
  let clanNewsCount = 0;
  const clanNews = (importPayload.clan_news ?? []).map((n) => ({
    id: n.id,
    title: n.title,
  ...
  if (clanNews.length > 0) {
    for (let i = 0; i < clanNews.length; i += CHUNK) {
      const chunk = clanNews.slice(i, i + CHUNK);
      const { error } = await supabase.from("clan_news").insert(chunk);
      if (error)
        return {
          error: `Lỗi khi import clan_news (chunk ${i / CHUNK + 1}): ${error.message}`,
        };
    }
    clanNewsCount = clanNews.length;
  }
