  ...
  for (const p of persons) {
    ...
    if (p.is_deceased && ((p.death_lunar_month && p.death_lunar_day) || (p.death_month && p.death_day))) {
      try {
        ...
        // Lễ tiên thường: trước ngày giỗ 01 ngày
        const reminderDate = new Date(next);
        reminderDate.setDate(reminderDate.getDate() - 1);
        const reminderDaysUntil = Math.round(
        ...
        events.push({
          ...
          eventDateLabel: `${lDay.toString().padStart(2, "0")}/${lMonth.toString().padStart(2, "0")} ÂL (Tiên thường)`,
          content: "Lễ tiên thường trước ngày giỗ 01 ngày",
        });
